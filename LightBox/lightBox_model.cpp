...
namespace lightBox {
	...
	LightBoxModel::LightBoxModel(LightBoxDevice & device, const std::vector<Vertex>& vertices) : lightBoxDevice(device)
	{
	...
	void LightBoxModel::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		lightBoxDevice.createBuffer(
			...
			vertexBufferMemory);
		void *data;
		vkMapMemory(lightBoxDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);

		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
		vkUnmapMemory(lightBoxDevice.device(), vertexBufferMemory);
	}
